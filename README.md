...
| ✈️ | **Flight search** | IATA autocomplete for airports, one-way and round-trip search, normalised offer cards with airline name, stops, times and price. |
...
| 🗺️ | **Hotel map** | Search a place, pan the map, and see accommodation from OpenStreetMap as markers — filtered and sorted by real distance from the map centre. |
